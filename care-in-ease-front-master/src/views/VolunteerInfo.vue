<template>
  <div id="VolunteerInfo" class="box">

    <el-main>
      <el-breadcrumb separator-class="el-icon-arrow-right" style="background: #CC9999;padding-bottom: 4px;margin-bottom: 30px">
          ...
          <el-table-column prop="profile_photo" label="头像">
            <template slot-scope="scope">
              <el-image
                  ...
                  @click="imginto(scope.$index, scope.row, 'volunteer')"></el-image>
            </template>
          </el-table-column>
...
<!--          <el-table-column prop="room_number" width="80px" label="房间号"></el-table-column>-->
          ...
          <el-table-column prop="checkin_date" label="访问日期" :formatter="dateFormat" sortable></el-table-column>
          <el-table-column prop="checkout_date" label="离开日期" :formatter="dateFormat" sortable></el-table-column>

          <el-table-column label="编辑">
            <template slot-scope="scope">
              <router-link to="/VolunteerDetails">
...
name: "VolunteerInfo",
  ...
  methods: {
    ...
    listAllInfo() {
      ...
      this.$http.get(this.global.baseURL + this.global.volunteerURL.getAll).then(result => {
    ...
    deleteInfo(index, row) {
      this.$http.delete(this.global.baseURL + this.global.volunteerURL.delete + "/" + row.id).then(result => {
